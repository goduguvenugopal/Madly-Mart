...
const OrderOverView = () => {
  ...
  const cancelOrderFunc = async (updataStatus) => {
    ...
    if (
      ...
      toast.warning(
        `Orders cannot be canceled once ${singleOrder.orderStatus.replace(
          "outofdelivery",
          "out of delivery"
      ...
      try {
        ...
        const res = await axios.put(
          `${api}/api/order/update-order-status/${singleOrder._id}`,
          updataStatus
        );
        if (res) {
      ...
      <div className="mt-20 px-3 lg:px-10 pt-4 pb-10 ">
        ...
        <div className="flex flex-wrap gap-5 mt-6">
          ...
          <div className="flex flex-col w-full lg:w-[45%] items-start gap-2 pb-3 border-b border-gray-400 ">
            {/* Product Image */}
            <Link
              to={`/product_over_view/${singleOrder?.orderedProdcuts[0]?.productId}`}
              className="flex gap-5 capitalize"
            >
              <div className="w-[6.8rem] h-fit lg:w-[9.5rem]">
                <img
                  src={
                    singleOrder?.orderedProdcuts[0]?.products[0]?.itemImage[0]
                      ?.image
                  }
              ...
              <div>
                <h2 className="text-sm lg:text-lg font-semibold text-black">
                  {singleOrder?.orderedProdcuts[0]?.products[0]?.itemName.substring(
                    0,
                    30
                  )}
                  ...
                ...
                <h2 className="text-sm lg:text-lg font-semibold text-black">
                  Rs.{" "}
                  {singleOrder?.orderedProdcuts[0]?.totalAmount *
                    singleOrder?.orderedProdcuts[0]?.itemQty}
                </h2>

                {singleOrder?.orderedProdcuts[0]?.size && (
                  <h2 className="text-sm lg:text-lg font-semibold text-black  ">
                    size : {singleOrder?.orderedProdcuts[0]?.size}
                  </h2>
                )}

                {singleOrder?.orderedProdcuts[0]?.color && (
                  <h2 className="text-sm lg:text-lg font-semibold text-black  ">
                    {singleOrder?.orderedProdcuts[0]?.color}
                  </h2>
                )}

                {singleOrder?.orderedProdcuts[0]?.capacity && (
                  <h2 className="text-sm lg:text-lg font-semibold text-black  ">
                    {singleOrder?.orderedProdcuts[0]?.capacity}
                  </h2>
                )}

                {singleOrder?.orderedProdcuts[0]?.weight && (
                  <h2 className="text-sm lg:text-lg font-semibold text-black  ">
                    {singleOrder?.orderedProdcuts[0]?.weight}
                  </h2>
                )}
            ...
            <span className="text-gray-700 font-medium">
              Order Date : {singleOrder.orderDate}
            </span>
            ...
            {singleOrder?.orderedProdcuts.length > 1 && (
              <>
                ...
                <details className="flex flex-col gap-3 w-full">
                  <summary className="text-gray-600 cursor-pointer text-sm">
                    See products ordered together
                  </summary>
                  {singleOrder?.orderedProdcuts.slice(1).map((item) => (
                    <Link
                      to={`/product_over_view/${item.productId}`}
                      className="flex gap-5 pb-3 mt-3 border-b border-gray-500"
                      key={item._id}
                    >
                      <div className="w-[6.8rem] h-fit lg:w-[9.5rem]">
                        <img
            ...
            {singleOrder.orderStatus === "cancelled" ? (
              <div className="bg-gray-100  rounded flex items-center gap-3 p-3">
                ...
                <div className=" rounded-md ">
                  <div className="font-bold text-red-500 text-[0.9rem] lg:text-lg">
                    Order Cancelled
                  </div>
              ...
              <div className="space-y-2 ">
                ...
                {singleOrder.orderStatus === "pending" ? (
                  <div className="bg-red-200 text-black rounded flex items-center gap-3 p-3">
                    <Lottie
                      className="w-14  lg:w-16  bg-red-200 rounded-full"
            ...
            singleOrder.orderStatus === "confirmed" ? (
              <>
                {singleOrder.delayMessage && (
                  <p className="text-sm my-3 font-medium text-red-600">
                    <strong>Note : </strong>
                    {singleOrder?.delayMessage}
                  </p>
                )}
              ...
              >
                ...
                >
                  ...
                  <div className="flex justify-end gap-2">
                    <button
                      onClick={() =>
                        cancelOrderFunc({ orderStatus: "cancelled" })
                      }
          ...
          <div className="w-full lg:w-[45%]">
            ...
            <div className="flex border-t pt-2 flex-col text-black  bg-white relative  items-start gap-1 mb-3 ">
              ...
              <h6 className="capitalize">
                {singleOrder?.shippingAddress[0].phone}{" "}
              </h6>
              <h6 className="">{singleOrder?.shippingAddress[0].email} </h6>
              <h6 className="capitalize">
                {singleOrder?.shippingAddress[0].village},{" "}
                {singleOrder?.shippingAddress[0].district}
              </h6>
              <h6 className="capitalize">
                {singleOrder?.shippingAddress[0].street}{" "}
              </h6>
              <h6 className=" capitalize">
                {singleOrder?.shippingAddress[0].state},{" "}
                {singleOrder?.shippingAddress[0].postalCode}{" "}
              </h6>
            </div>
          </div>
